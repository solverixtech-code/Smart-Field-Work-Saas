...
export function ModuleSelectionGrid({
  ...
  // Map modules into 3 Canonical Categories
  ...
  }, [modules, mandatoryCodes]);

  const allSelectableCodes = React.useMemo(() => {
  ...
  const summaryCounts = React.useMemo(() => {
    return categoryGroups.map((group) => {
      const enabled = group.items.filter((m) => selectedCodes.includes(m.code)).length;
      return {
        title: group.title,
        enabled,
  ...
  return (
    <div className="space-y-6 font-sans">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between rounded-sm border border-slate-200 bg-white p-5 shadow-xs">
        ...
        <div className="rounded-sm bg-purple-50 px-3.5 py-2 text-xs text-purple-900 font-semibold border border-purple-100/80 flex items-center gap-2.5 shrink-0 max-w-md">
