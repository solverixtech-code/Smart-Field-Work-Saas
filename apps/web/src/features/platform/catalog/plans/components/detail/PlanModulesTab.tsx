...
export function PlanModulesTab({ plan, includedModules }: PlanModulesTabProps) {
  ...
  // Module category icon mapping
  const getModuleIcon = (code: string, category: string) => {
    if (code.includes('crm') || category === 'Core') return { icon: Users, bg: 'bg-blue-50 text-blue-600' };
    if (code.includes('visit') || category === 'Field Ops') return { icon: MapPin, bg: 'bg-emerald-50 text-emerald-600' };
    if (code.includes('order') || category === 'Sales') return { icon: ShoppingCart, bg: 'bg-amber-50 text-amber-600' };
    if (code.includes('collection') || code.includes('payroll')) return { icon: Receipt, bg: 'bg-purple-50 text-purple-600' };
  ...
  const filteredModules = useMemo(() => {
    return includedModules.filter((mod) => {
      ...
      let matchType = true;
      if (selectedType === 'System Required') {
        matchType = !!mod.requiredBySystem;
      } else if (selectedType === 'Standard') {
        matchType = !mod.requiredBySystem && !mod.isAddon;
      } else if (selectedType === 'Add-on') {
        matchType = !!mod.isAddon;
      }
  ...
  const coreModules = useMemo(
    () => filteredModules.filter((m) => m.requiredBySystem || m.category === 'Core'),
    [filteredModules]
  );
    ...
    <div className="space-y-6 font-sans w-full max-w-full overflow-hidden">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between rounded-sm border border-slate-200 bg-white p-5 shadow-xs">
        ...
        <Button
          ...
          className="gap-2 font-bold shadow-xs shrink-0 bg-[#1D4ED8] hover:bg-blue-700 text-white border-none cursor-pointer"
        >
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
        ...
        <div className="lg:col-span-2 space-y-6 min-w-0">
          ...
          <div className="rounded-sm border border-slate-200 bg-white p-4 shadow-xs space-y-3">
            <div className="flex flex-wrap items-center justify-between gap-3">
              ...
              <div className="w-full sm:w-40">
                <Select
                  ...
                  options={[
                    ...
                    { value: 'System Required', label: 'System Required' },
              ...
              <Button
                ...
                className="gap-1.5 font-bold text-slate-700 h-10 border-slate-200 cursor-pointer"
              >
          ...
          {coreModules.length > 0 && (
            <div className="space-y-3 min-w-0">
              ...
              <div className="space-y-3">
                {coreModules.map((mod) => {
                  const iconConfig = getModuleIcon(mod.code, mod.category);
                  ...
                  return (
                    <div
                      key={mod.id}
                      className="rounded-sm border border-slate-200 bg-white p-4 shadow-xs space-y-3 min-w-0 overflow-hidden"
                    >
                        ...
                        <div className="flex items-start gap-3 min-w-0 flex-1">
                          ...
                          <div className="space-y-1 min-w-0 flex-1">
                            <div className="flex flex-wrap items-center gap-2">
                              <span className="text-sm font-extrabold text-[#0D1F3D] break-words">{mod.name}</span>
                              <span className="inline-flex rounded-full bg-emerald-50 px-2 py-0.5 text-[10px] font-extrabold text-emerald-700 border border-emerald-200 shrink-0">
                                Core
                              </span>
                            </div>
                            <p className="text-xs text-slate-600 font-medium leading-relaxed break-words">{mod.description}</p>
                            <p className="text-[11px] text-slate-500 font-medium break-words">
                              Code: <span className="font-mono font-bold text-slate-800">{mod.code.toUpperCase()}</span> • Category: <span className="text-slate-800 font-semibold">{mod.category}</span>
                            </p>
                          </div>
                        </div>
                      ...
                      {isExpanded && (
                        <div className="pt-3 border-t border-slate-100 text-xs space-y-2 bg-slate-50/50 p-3 rounded-sm min-w-0">
                          <p className="text-slate-700 font-medium break-words">{mod.description || 'No detailed description provided.'}</p>
                          <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 pt-1 font-mono text-[11px] break-words">
                            <div><span className="text-slate-500 font-sans">Module Code:</span> {mod.code}</div>
                            <div><span className="text-slate-500 font-sans">Category:</span> {mod.category}</div>
                            <div><span className="text-slate-500 font-sans">System Required:</span> {mod.requiredBySystem ? 'Yes' : 'No'}</div>
                            <div><span className="text-slate-500 font-sans">Add-on Eligible:</span> {mod.isAddon ? 'Yes' : 'No'}</div>
                          </div>
                        </div>
                      )}
                    </div>
          ...
          {advancedModules.length > 0 && (
            <div className="space-y-3 min-w-0">
              <h4 className="text-xs font-extrabold text-[#0D1F3D]">
                Advanced & Standard Modules ({advancedModules.length})
              </h4>
              <div className="space-y-3">
                      ...
                      <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-3">
                        <div className="flex items-start gap-3 min-w-0 flex-1">
                          ...
                          <div className="space-y-1 min-w-0 flex-1">
                            <div className="flex flex-wrap items-center gap-2">
                              <span className="text-sm font-extrabold text-[#0D1F3D] break-words">{mod.name}</span>
                              <span className="inline-flex rounded-full bg-blue-50 px-2 py-0.5 text-[10px] font-extrabold text-blue-700 border border-blue-200 shrink-0">
                                {mod.category}
                              </span>
                            </div>
                            <p className="text-xs text-slate-600 font-medium leading-relaxed break-words">{mod.description}</p>
                      ...
                      {isExpanded && (
                        <div className="pt-3 border-t border-slate-100 text-xs space-y-2 bg-slate-50/50 p-3 rounded-sm min-w-0">
                          ...
                          <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 pt-1 font-mono text-[11px] break-words">
                            ...
                            <div><span className="text-slate-500 font-sans">Category:</span> {mod.category}</div>
                            <div><span className="text-slate-500 font-sans">Add-on Eligible:</span> {mod.isAddon ? 'Yes' : 'No'}</div>
                            <div><span className="text-slate-500 font-sans">Monthly Addon Price:</span> {mod.monthlyPrice ? `₹${mod.monthlyPrice}` : 'N/A'}</div>
                          </div>
                        </div>
                      )}
                    </div>
        ...
        <div className="space-y-6 min-w-0">
          ...
          <div className="rounded-sm border border-slate-200 bg-white p-5 shadow-xs space-y-4 min-w-0">
            ...
            <div className="space-y-2.5 text-xs">
              <div className="flex items-center justify-between">
                <span className="text-xs font-semibold text-slate-500">Total Included</span>
              ...
              <div className="flex items-center justify-between">
                <span className="text-xs font-semibold text-slate-500">Advanced / Standard</span>
          ...
          <div className="rounded-sm border border-slate-200 bg-white p-5 shadow-xs space-y-3 min-w-0">
            ...
            <div className="space-y-2 text-xs">
              {['Core', 'Sales', 'Field Ops', 'Automation', 'Enterprise'].map((cat) => (
                <div key={cat} className="flex items-center justify-between p-2 rounded-sm bg-slate-50/50">
                  <span className="text-slate-700 font-semibold flex items-center gap-2">
                    <Layers className="h-3.5 w-3.5 text-indigo-600" /> {cat}
                  </span>
                  <span className="font-extrabold text-[#0D1F3D]">{categoryCounts[cat] || 0}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="rounded-sm border border-slate-200 bg-white p-5 shadow-xs space-y-3 min-w-0">
            ...
            <div className="space-y-2.5 text-xs text-slate-600 font-medium">
              <div className="flex items-start gap-2">
                ...
                <span className="break-words">All included modules are available to tenants based on their subscription.</span>
              </div>
              <div className="flex items-start gap-2">
                <Info className="h-4 w-4 text-blue-600 shrink-0 mt-0.5" />
                <span className="break-words">Module limits are defined in the Limits tab.</span>
              </div>
              <div className="flex items-start gap-2">
                <ShieldCheck className="h-4 w-4 text-purple-600 shrink-0 mt-0.5" />
                <span className="break-words">System required modules cannot be removed.</span>
              </div>
            </div>
