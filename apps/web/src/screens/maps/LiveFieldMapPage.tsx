    ...
    <div className="space-y-4 font-sans pb-8 text-left">
      ...
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-6">
        <MapKpiCard
          ...
          subValue={`${summary?.totalExecutives ? Math.round((summary.activeExecutives / summary.totalExecutives) * 100) : 0}% active`}
          icon={Users}
        ...
        <MapKpiCard
          ...
          subValue={`Completed: ${summary?.completedVisits ?? 0} (${summary?.visits ? Math.round((summary.completedVisits / summary.visits) * 100) : 0}%)`}
          icon={CalendarCheck}
        ...
        <div className="space-y-4 lg:col-span-4 flex flex-col">
          ...
          <div className="rounded-sm border border-slate-200/90 bg-white p-4 shadow-sm space-y-3 flex-1">
            <div className="flex items-center justify-between border-b border-slate-100 pb-2">
              <h3 className="font-extrabold text-[#0D1F3D] text-xs">Live Executives ({summary?.activeExecutives ?? 0})</h3>
              ...
              {filteredExecutives.map((exec) => (
                ...
                >
                  <div className="flex items-center gap-2.5 min-w-0">
                    <img src={mapAvatar(exec.name, exec.avatar)} alt={exec.name} className="h-8 w-8 rounded-full object-cover border shrink-0" />
                  ...
                  <div className="text-right shrink-0">
                    <span className="text-[10px] text-slate-400 font-medium block">{mapDate(exec.lastUpdatedAt, data?.timezone)}</span>
          ...
          <div className="rounded-sm border border-slate-200/90 bg-white p-4 shadow-sm space-y-3">
            ...
            <div className="space-y-2.5 text-xs">
              {(data?.activities ?? []).slice(0, showAllActivities ? undefined : 3).map((activity, index) => (
                <div key={activity.id} className={`flex items-start justify-between gap-2 ${index < Math.min(2, (data?.activities.length ?? 0) - 1) ? 'border-b border-slate-100 pb-2' : ''}`}>
                  <div className="flex items-start gap-2">
                    <img src={mapAvatar(activity.executiveName, activity.avatar)} alt={activity.executiveName} className="h-6 w-6 rounded-full object-cover border mt-0.5" />
                    <div>
                      <p className="font-bold text-[#0D1F3D]">{activity.executiveName} <span className="text-emerald-600 font-semibold">• {activity.action}</span></p>
                      <p className="text-[11px] text-slate-500 font-medium">{activity.location}</p>
                    </div>
                  </div>
                  <span className="text-[10px] text-slate-400 font-medium shrink-0">{mapDate(activity.occurredAt, data?.timezone)}</span>
                </div>
              ))}
            </div>
          </div>
