...
function WizardContent() {
  ...
  if (planNotFound) {
    return (
      <main className="flex-1 overflow-y-auto bg-[#F3F5F7]">
        <div className="mx-auto max-w-md p-12 text-center font-sans space-y-4">
          <AlertTriangle className="h-10 w-10 text-amber-500 mx-auto" />
          <h2 className="text-lg font-extrabold text-[#0D1F3D]">Plan Not Found</h2>
          <p className="text-xs text-slate-500 font-medium">
            The requested plan configuration ID does not exist or has been removed.
          </p>
          <Button variant="accent" size="sm" onClick={() => navigate('/platform/plans')} className="gap-2">
  ...
  const steps = [
    { num: 1, name: 'Basic Information', desc: 'Plan details & identity', icon: Crown },
    { num: 2, name: 'Pricing', desc: 'Prices & billing model', icon: DollarSign },
    { num: 3, name: 'Limits', desc: 'Seats & usage limits', icon: Sliders },
    { num: 4, name: 'Modules', desc: 'Included modules', icon: Package },
    { num: 5, name: 'Trial & Rules', desc: 'Trial & commercial rules', icon: ShieldCheck },
    { num: 6, name: 'Review', desc: 'Review & publish', icon: Eye },
  ];
  ...
  return (
    <main className="flex-1 overflow-y-auto bg-[#F3F5F7]">
      <div className="mx-auto w-full max-w-[1720px] p-6 lg:p-8 space-y-6 font-sans">
          ...
          <div>
            <div className="flex items-center gap-1.5 text-xs font-semibold text-slate-500">
              ...
              <span className="font-extrabold text-[#0D1F3D]">
                {isEditMode ? `Edit Plan — ${formState.name || formState.code}` : 'Create New Plan'}
              </span>
            </div>
            <h1 className="text-2xl font-extrabold text-[#0D1F3D] tracking-tight mt-1">
              {isEditMode ? `Edit Commercial Plan — ${formState.name}` : 'Create Commercial Plan'}
            </h1>
            <p className="text-xs text-slate-500 font-medium mt-0.5">
              Configure a commercial subscription plan for the Smart Field Work SaaS platform.
            </p>
          </div>
            ...
            {steps.map((step) => {
              ...
              return (
                <button
                  ...
                  type="button"
                  onClick={() => {
                    if (isCompleted || isActive) updateStepInUrl(step.num);
                  }}
                  className={`flex items-center gap-3 p-2.5 rounded-sm border text-left transition-all cursor-pointer ${
                    isActive
                ...
                >
                  <div
                    className={`h-7 w-7 rounded-sm flex items-center justify-center font-extrabold text-xs shrink-0 ${
                      isActive
                  ...
                  <div className="min-w-0">
                    <div className={`text-xs font-extrabold truncate ${isActive ? 'text-[#0D1F3D]' : 'text-slate-700'}`}>
                      {step.name}
                    </div>
        ...
        {/* MAIN FORM GRID: Form (8 cols) + Right Summary (4 cols) */}
        <div className="grid grid-cols-1 gap-6 lg:grid-cols-12">
            ...
            <div className="rounded-sm border border-slate-200 bg-white p-6 shadow-xs space-y-6">
              ...
              {currentStep === 1 && (
                <div className="space-y-5">
                  ...
                  {/* Row 2: Customer Facing & Internal Description */}
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div className="space-y-1.5">
                  ...
                  {/* Row 3: Tier, Visibility, Display Order */}
                  ...
                  {/* Row 4: Recommended For, Badge, Plan Color */}
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                    <div className="space-y-1.5">
              ...
              {currentStep === 2 && (
                <div className="space-y-5">
                        ...
                        [
                          {
                            model: 'Per User',
                            title: 'Per User',
                            desc: 'Fixed rate per active seat license / month',
                          },
                          {
                            model: 'Base + Per User',
                            title: 'Base + Per User',
                            desc: 'Base platform fee plus seat rate',
                          },
                          {
                            model: 'Flat Monthly',
                            title: 'Flat Monthly',
                            desc: 'Fixed flat rate regardless of seat count',
                          },
                          {
                            model: 'Custom Contract',
                            title: 'Custom Contract',
                            desc: 'Contact Sales / Custom offline contract',
                          },
                        ] as const
                      ).map((item) => {
                  ...
                  {/* Dynamic Pricing Inputs Based on Selected Model */}
              ...
              {currentStep === 6 && (
                <div className="space-y-6">
                  ...
                  {/* Summary Card A: Basic Information */}
                ...
                <div className="flex items-center gap-3">
                  {currentStep > 1 && (
                    <Button
                      variant="outline"
                      size="sm"
                      onClick={() => updateStepInUrl(currentStep - 1)}
                  ...
                  {currentStep < 6 ? (
                    ...
                    <Button
                      ...
                      className="gap-2 font-extrabold bg-emerald-600 hover:bg-emerald-700 text-white shadow-xs"
                    >
                      <CheckCircle2 className="h-4 w-4" />
                      {isPublishing ? 'Publishing Plan...' : 'Publish Plan'}
                    </Button>
                  )}
                </div>
              </div>
          ...
          {/* Right Live Plan Summary Column */}
          <div className="lg:col-span-4 space-y-4">
            <div className="rounded-sm border border-slate-200 bg-white p-5 shadow-xs space-y-4 sticky top-6">
              <div className="flex items-center justify-between border-b border-slate-100 pb-3">
                <h3 className="text-xs font-extrabold text-[#0D1F3D] uppercase tracking-wider flex items-center gap-2">
                  <Crown className="h-4 w-4 text-indigo-600 shrink-0" /> Live Plan Summary
                </h3>
              ...
              <div className="space-y-3 text-xs">
                ...
                <div className="rounded-sm bg-indigo-50/60 p-3 border border-indigo-100 text-indigo-900 text-xs font-medium space-y-1 mt-2">
          ...
          <div className="w-full max-w-md rounded-sm border border-slate-200 bg-white p-6 shadow-xl space-y-4">
            <div className="flex items-center gap-3 text-amber-600">
              ...
              <div>
                ...
                <p className="text-xs text-slate-500 font-medium">Discard changes and exit wizard?</p>
              </div>
            </div>

            <p className="text-xs text-slate-600 font-medium leading-relaxed">
              You have unsaved changes in this commercial plan configuration. If you exit now, your changes will be lost unless saved as a draft.
            </p>
