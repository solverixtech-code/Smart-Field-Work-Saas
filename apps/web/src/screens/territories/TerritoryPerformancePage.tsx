    ...
    <div className="space-y-4 font-sans pb-16 bg-slate-50/50 min-h-screen p-1 sm:p-2 text-left">
        ...
        <div className="space-y-4 text-xs font-semibold">
          <div className="grid grid-cols-1 sm:grid-cols-4 gap-3">
            <div className="rounded-sm border border-slate-200/80 bg-white p-4 shadow-xs">
              <span className="text-[10px] text-slate-400 font-bold block">Total Visits Logged</span>
              <span className="text-xl font-extrabold text-[#0D1F3D]">176</span>
            </div>
            <div className="rounded-sm border border-slate-200/80 bg-white p-4 shadow-xs">
              <span className="text-[10px] text-slate-400 font-bold block">Completed Visits</span>
              <span className="text-xl font-extrabold text-emerald-600">142</span>
            </div>
            <div className="rounded-sm border border-slate-200/80 bg-white p-4 shadow-xs">
              <span className="text-[10px] text-slate-400 font-bold block">Missed / Unverified</span>
              <span className="text-xl font-extrabold text-red-600">34</span>
            </div>
            <div className="rounded-sm border border-slate-200/80 bg-white p-4 shadow-xs">
              <span className="text-[10px] text-slate-400 font-bold block">Lead Conversion Rate</span>
              <span className="text-xl font-extrabold text-purple-600">28.6%</span>
            </div>
          </div>
          ...
          <div className="rounded-sm border border-slate-200 bg-white p-4 shadow-xs space-y-3">
            ...
            <table className="w-full text-left border-collapse">
              ...
              <tbody className="divide-y divide-slate-100">
                {[
                  { time: '20 May, 10:30 AM', exec: 'Arjun Mehta', business: 'Sai Enterprises', purpose: 'Product Demo', status: 'Completed', ver: 'GPS Verified' },
                  { time: '20 May, 11:45 AM', exec: 'Neha Sharma', business: 'Sharma Medical', purpose: 'Payment Collection', status: 'Completed', ver: 'GPS Verified' },
                  { time: '20 May, 02:15 PM', exec: 'Pooja Yadav', business: 'Marol Electronics', purpose: 'Lead Follow-up', status: 'Completed', ver: 'GPS Verified' },
                  { time: '19 May, 04:30 PM', exec: 'Rahul Verma', business: 'Apex Traders', purpose: 'Store Onboarding', status: 'Missed', ver: 'Unverified' },
                ].map((v, i) => (
                  <tr key={i} className="hover:bg-slate-50">
                    ...
                    <td className="p-2.5 text-center">
                      <span className={`px-2 py-0.5 rounded-full text-[10px] font-bold ${v.status === 'Completed' ? 'bg-emerald-50 text-emerald-700' : 'bg-red-50 text-red-700'}`}>
                        • {v.status}
                      </span>
                    </td>
                    <td className="p-2.5 text-center text-slate-500 font-mono text-[10px]">{v.ver}</td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      {activeSubTab === 'Sales & Revenue' && (
        <div className="space-y-4 text-xs font-semibold">
          <div className="grid grid-cols-1 sm:grid-cols-4 gap-3">
            <div className="rounded-sm border border-slate-200/80 bg-white p-4 shadow-xs">
              <span className="text-[10px] text-slate-400 font-bold block">Total Revenue</span>
              <span className="text-xl font-extrabold text-emerald-600">₹ 14,00,000</span>
            </div>
            <div className="rounded-sm border border-slate-200/80 bg-white p-4 shadow-xs">
              <span className="text-[10px] text-slate-400 font-bold block">Deals Closed</span>
              <span className="text-xl font-extrabold text-[#0D1F3D]">28</span>
            </div>
            <div className="rounded-sm border border-slate-200/80 bg-white p-4 shadow-xs">
              <span className="text-[10px] text-slate-400 font-bold block">Avg. Deal Value</span>
              <span className="text-xl font-extrabold text-blue-600">₹ 50,000</span>
            </div>
            <div className="rounded-sm border border-slate-200/80 bg-white p-4 shadow-xs">
              <span className="text-[10px] text-slate-400 font-bold block">Target Achievement</span>
              <span className="text-xl font-extrabold text-amber-600">106%</span>
            </div>
          </div>
          ...
          <div className="rounded-sm border border-slate-200 bg-white p-4 shadow-xs space-y-3">
            ...
            <table className="w-full text-left border-collapse">
              ...
              <tbody className="divide-y divide-slate-100">
                {[
                  { id: 'DEAL-901', name: 'Sai Enterprises', exec: 'Arjun Mehta', pkg: 'Enterprise POS', amount: '₹ 2,48,000', method: 'UPI / Bank' },
                  { id: 'DEAL-902', name: 'Sharma Medical', exec: 'Neha Sharma', pkg: 'Pharma Pro Plan', amount: '₹ 1,96,000', method: 'Cheque' },
                  { id: 'DEAL-903', name: 'Marol Electronics', exec: 'Pooja Yadav', pkg: 'Retail Starter', amount: '₹ 1,58,000', method: 'Online Transfer' },
                  { id: 'DEAL-904', name: 'Apex Traders', exec: 'Rahul Verma', pkg: 'ERP Sync Pack', amount: '₹ 1,20,000', method: 'UPI' },
                ].map((d) => (
                  <tr key={d.id} className="hover:bg-slate-50">
                    <td className="p-2.5 font-mono text-slate-500">{d.id}</td>
                    <td className="p-2.5 font-extrabold text-[#0D1F3D]">{d.name}</td>
                    <td className="p-2.5">{d.exec}</td>
                    <td className="p-2.5 text-slate-600">{d.pkg}</td>
                    <td className="p-2.5 text-right font-mono font-bold text-emerald-700">{d.amount}</td>
                    <td className="p-2.5 text-center text-slate-500 font-mono text-[10px]">{d.method}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <div className="space-y-4 text-xs font-semibold">
          <div className="grid grid-cols-1 sm:grid-cols-4 gap-3">
            <div className="rounded-sm border border-slate-200/80 bg-white p-4 shadow-xs">
              ...
              <span className="text-xl font-extrabold text-[#0D1F3D]">₹ 15,20,000</span>
            </div>
            <div className="rounded-sm border border-slate-200/80 bg-white p-4 shadow-xs">
              <span className="text-[10px] text-slate-400 font-bold block">Collected</span>
              <span className="text-xl font-extrabold text-emerald-600">₹ 14,00,000</span>
            </div>
            <div className="rounded-sm border border-slate-200/80 bg-white p-4 shadow-xs">
              <span className="text-[10px] text-slate-400 font-bold block">Outstanding</span>
              <span className="text-xl font-extrabold text-amber-600">₹ 1,20,000</span>
            </div>
            <div className="rounded-sm border border-slate-200/80 bg-white p-4 shadow-xs">
              <span className="text-[10px] text-slate-400 font-bold block">Collection Efficiency</span>
              <span className="text-xl font-extrabold text-blue-600">92.1%</span>
            </div>
          </div>
          ...
          <div className="rounded-sm border border-slate-200 bg-white p-4 shadow-xs space-y-3">
            ...
            <table className="w-full text-left border-collapse">
              ...
              <tbody className="divide-y divide-slate-100">
                {[
                  { inv: 'INV-2025-081', name: 'Sai Enterprises', due: '15 May 2025', amount: '₹ 2,48,000', status: 'Paid' },
                  { inv: 'INV-2025-082', name: 'Sharma Medical', due: '18 May 2025', amount: '₹ 1,96,000', status: 'Paid' },
                  { inv: 'INV-2025-083', name: 'Marol Supermarket', due: '22 May 2025', amount: '₹ 85,000', status: 'Pending' },
                  { inv: 'INV-2025-084', name: 'Metro Clinic', due: '10 May 2025', amount: '₹ 35,000', status: 'Overdue' },
                ].map((row) => (
                  <tr key={row.inv} className="hover:bg-slate-50">
                    <td className="p-2.5 font-mono text-slate-500">{row.inv}</td>
                    <td className="p-2.5 font-extrabold text-[#0D1F3D]">{row.name}</td>
                    <td className="p-2.5 font-mono text-slate-600">{row.due}</td>
                    <td className="p-2.5 text-right font-mono font-bold">{row.amount}</td>
                    <td className="p-2.5 text-center">
                      <span className={`px-2 py-0.5 rounded-full text-[10px] font-bold ${row.status === 'Paid' ? 'bg-emerald-50 text-emerald-700' : row.status === 'Pending' ? 'bg-amber-50 text-amber-700' : 'bg-red-50 text-red-700'}`}>
                        • {row.status}
                      </span>
                    </td>
                    <td className="p-2.5 text-center">
                      <Button variant="outline" size="sm" onClick={() => toast.info(`Reminder sent for ${row.inv}`)} className="text-[10px] py-0.5 px-2">
                        Send Reminder
                      </Button>
                    </td>
                  </tr>
      ...
      {activeSubTab === 'Demos & Conversions' && (
        <div className="space-y-4 text-xs font-semibold">
          <div className="grid grid-cols-1 sm:grid-cols-4 gap-3">
            <div className="rounded-sm border border-slate-200/80 bg-white p-4 shadow-xs">
              <span className="text-[10px] text-slate-400 font-bold block">Demos Scheduled</span>
              <span className="text-xl font-extrabold text-[#0D1F3D]">45</span>
            </div>
            <div className="rounded-sm border border-slate-200/80 bg-white p-4 shadow-xs">
              <span className="text-[10px] text-slate-400 font-bold block">Demos Conducted</span>
              <span className="text-xl font-extrabold text-emerald-600">36</span>
            </div>
            <div className="rounded-sm border border-slate-200/80 bg-white p-4 shadow-xs">
              <span className="text-[10px] text-slate-400 font-bold block">Deals Converted</span>
              <span className="text-xl font-extrabold text-purple-600">28</span>
            </div>
            <div className="rounded-sm border border-slate-200/80 bg-white p-4 shadow-xs">
              <span className="text-[10px] text-slate-400 font-bold block">Demo Win Rate</span>
              <span className="text-xl font-extrabold text-amber-600">77.7%</span>
            </div>
          </div>
              ...
              <tbody className="divide-y divide-slate-100">
                {[
                  ...
                  <tr key={row.name} className="hover:bg-slate-50">
                    ...
                    <td className="p-2.5 text-center font-bold text-emerald-600">{row.closed}</td>
                    <td className="p-2.5 text-center font-extrabold text-blue-600">{row.rate}</td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      {activeSubTab === 'Performance by Executive' && (
        <div className="rounded-sm border border-slate-200 bg-white p-4 shadow-xs space-y-3 text-xs font-semibold">
          <h3 className="text-xs font-extrabold text-[#0D1F3D] border-b border-slate-100 pb-2">
            Territory Executive Leaderboard ({mockTerritoryExecutives.length} Members)
          </h3>
            ...
            <tbody className="divide-y divide-slate-100">
              {mockTerritoryExecutives.map((exec) => (
                <tr key={exec.id} className="hover:bg-slate-50">
                  ...
                  <td className="p-3 text-center font-bold">{Math.floor(exec.visitsCount * 0.6)}</td>
                  ...
                  <td className="p-3 text-center">
                    <span className={`px-2 py-0.5 rounded-full text-[10px] font-bold ${exec.performancePercentage >= 95 ? 'bg-amber-100 text-amber-800' : 'bg-emerald-50 text-emerald-700'}`}>
                      {exec.performancePercentage >= 95 ? '⭐ Top Performer' : 'On Track'}
                    </span>
                  </td>
                </tr>
              ))}
          ...
          <div className="h-48 w-full flex items-end justify-between gap-3 pt-6 px-4 bg-slate-50/50 rounded-sm">
            {[
              ...
              <div key={m.month} className="flex-1 flex flex-col items-center gap-1">
                <span className="text-[10px] font-bold text-slate-600">₹ {m.rev}L</span>
                <div
                  className="w-full bg-indigo-600 rounded-t-xs hover:bg-indigo-700 transition-all"
                  style={{ height: `${m.rev * 9}px` }}
                />
                <span className="text-[10px] text-slate-400 font-bold mt-1">{m.month}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="rounded-sm border border-slate-200 bg-white p-4 shadow-xs space-y-3 text-xs font-semibold">
          ...
          <div className="space-y-3">
            {[
              ...
              { time: '35 mins ago', title: 'Visit Completed', desc: 'Neha Sharma completed visit at Sharma Medical (GPS Verified)', icon: CheckCircle2, color: 'text-blue-600 bg-blue-50' },
              { time: '1 hour ago', title: 'New Lead Added', desc: 'Pooja Yadav added Marol Electronics as new prospect', icon: Users, color: 'text-purple-600 bg-purple-50' },
              { time: '2 hours ago', title: 'Boundary Updated', desc: 'Vikram Singh updated Andheri East boundary polygon', icon: MapPin, color: 'text-amber-600 bg-amber-50' },
            ].map((act, idx) => {
              const Icon = act.icon;
              return (
