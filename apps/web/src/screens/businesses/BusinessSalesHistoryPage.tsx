...
export default function BusinessSalesHistoryPage() {
  ...
  return (
    <div className="space-y-4 font-sans">
      ...
      {/* Main Content Grid: Invoices Table (9 Cols) + Sales Summary Sidebar (3 Cols) */}
        ...
        <div className="lg:col-span-9">
          <div className="overflow-hidden rounded-md border border-slate-200/80 bg-white shadow-sm flex flex-col justify-between">
              ...
              <table className="w-full text-left text-xs border-collapse">
                <thead>
                  <tr className="border-b border-slate-200 bg-slate-100/90 text-xs font-bold text-[#0D1F3D]">
                    <th className="px-3.5 py-3 whitespace-nowrap">Order / Invoice ID</th>
                    <th className="px-3.5 py-3 whitespace-nowrap">Order Date</th>
                    <th className="px-3.5 py-3 whitespace-nowrap">Order Type</th>
                    <th className="px-3.5 py-3 whitespace-nowrap text-center">Items</th>
                    <th className="px-3.5 py-3 whitespace-nowrap text-right">Amount (₹)</th>
                    <th className="px-3.5 py-3 whitespace-nowrap text-right">Discount (₹)</th>
                    <th className="px-3.5 py-3 whitespace-nowrap text-right">Tax (₹)</th>
                    <th className="px-3.5 py-3 whitespace-nowrap text-right">Total (₹)</th>
                    <th className="px-3.5 py-3 whitespace-nowrap text-center">Payment Status</th>
                ...
                <tbody className="divide-y divide-slate-100 text-slate-700 font-semibold">
                  {filteredOrders.map((o) => (
                    <tr key={o.id} className="hover:bg-slate-50/80 transition-colors">
                      <td className="px-3.5 py-3 font-mono font-bold text-[#0D1F3D] whitespace-nowrap">{o.invoiceId}</td>
                      <td className="px-3.5 py-3 whitespace-nowrap text-[11px] text-slate-500">{o.orderDate}</td>
                      ...
                      <td className="px-3.5 py-3 text-center whitespace-nowrap">{o.itemsCount}</td>
                      <td className="px-3.5 py-3 text-right whitespace-nowrap">₹{o.amount.toLocaleString()}</td>
                      <td className="px-3.5 py-3 text-right whitespace-nowrap text-slate-500">₹{o.discount.toLocaleString()}</td>
                      <td className="px-3.5 py-3 text-right whitespace-nowrap text-slate-500">₹{o.tax.toLocaleString()}</td>
                      <td className="px-3.5 py-3 text-right whitespace-nowrap font-bold text-[#0D1F3D]">₹{o.total.toLocaleString()}</td>
            ...
            <div className="flex items-center justify-between border-t border-slate-200 bg-slate-50/50 px-4 py-2 text-xs text-slate-500">
              <span>Showing 1 to {filteredOrders.length} of 68 entries</span>
