  ...
  const columns: ColumnDef<BusinessItem>[] = [
    ...
    },
    {
      ...
      cell: (b) => (
        <RowActionsMenu
          items={[
            { label: 'View Details', icon: Eye, onClick: () => navigate(`/admin/businesses/${b.id}`) },
            { label: 'Edit Business', icon: Edit, onClick: () => navigate(`/admin/businesses/${b.id}/edit`) },
            { label: 'Contacts', icon: UserPlus, onClick: () => navigate(`/admin/businesses/${b.id}/contacts`) },
          ]}
        />
      ),
    },
