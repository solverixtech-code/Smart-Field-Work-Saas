          ...
          <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
            <div>
              <div className="flex items-center gap-2">
                ...
                <h3 className="text-base font-extrabold text-[#0D1F3D]">Live Users Interactive Map</h3>
              </div>
              <p className="text-xs text-slate-500">Search any user below or on map to center Mapbox engine on their GPS location</p>
            </div>
          </div>

          {/* Interactive Mapbox Map Engine */}
          <div className="relative h-[500px] w-full overflow-hidden rounded-sm border border-slate-200 shadow-inner">
            <InteractiveMap
              mode="live-executives"
              heightClassName="h-full"
              selectedExecutiveId={selectedUser.id}
              executives={liveUsers.map((u) => ({
                id: u.id,
                name: u.name,
                avatar: u.avatar,
                status: (u.status === 'In Visit' ? 'On Field' : u.status === 'Traveling' ? 'In Transit' : 'On Field') as any,
                currentLocation: u.location,
                lastUpdated: 'Just now',
                batteryLevel: parseInt(u.battery),
                lat: u.lat,
                lng: u.lng,
                phone: u.phone,
                team: 'Mumbai Sales',
