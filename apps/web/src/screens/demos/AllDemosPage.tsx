...
export default function AllDemosPage() {
  ...
  return (
    <div className="space-y-4 font-sans pb-16 bg-slate-50/50 min-h-screen p-1 sm:p-2 text-left">
          ...
          <table className="w-full text-left text-xs font-semibold border-collapse">
            <thead>
              <tr className="border-b border-slate-200 bg-slate-50/80 text-slate-600">
                ...
                <th className="p-3 whitespace-nowrap">Demo Date & Time</th>
                    ...
                    <td className="p-3 text-center whitespace-nowrap">
                      {d.nextAction && d.nextAction !== '—' ? (
                        <div>
                          ...
                          {d.nextActionDate && (
                            <span className="text-[9px] text-slate-500 font-medium">{d.nextActionDate}</span>
                          )}
                        </div>
                      ) : (
                        <span className="text-slate-300 font-mono">—</span>
      ...
      {/* BOTTOM ANALYTICS CARDS (Placed on the next line below table) */}
