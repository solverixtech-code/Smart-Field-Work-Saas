  ...
  const handleSaveRecord = () => {
    ...
    if (modalMode === 'add') {
      const newRecord: MasterRecordItem = {
        ...
        name: formName.trim(),
        code: formCode.trim().toUpperCase() || `${activeCategoryConfig.defaultCodePrefix}_${Date.now().toString().slice(-4)}`,
    ...
    <div className="space-y-6 font-sans pb-12">
      ...
      <header className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between border-b border-slate-200 pb-5">
        <div>
          <div className="flex items-center gap-2">
            <h1 className="text-2xl font-extrabold text-[#0D1F3D]">System Masters</h1>
            <Info className="h-4 w-4 text-[#0B2E6B]" />
          </div>
        ...
        <div className="flex flex-wrap items-center gap-3">
          <Button
            ...
            className="h-10 px-4 font-bold border-slate-200 text-slate-700 hover:bg-slate-50 shadow-none flex items-center gap-1.5"
          >
            <FileText className="h-4 w-4 text-slate-500" /> Audit Log
          </Button>
          <Button
            type="button"
            variant="accent"
            onClick={handleOpenAddModal}
            className="h-10 px-5 font-bold shadow-xs flex items-center gap-1.5"
          >
      ...
      <div className="flex flex-col gap-6 lg:flex-row lg:items-start">
        ...
        <div className="flex flex-col gap-4">
              ...
              {masterCategories.map((cat) => {
                ...
                return (
                  <button
                    ...
                    className={`group flex w-full items-center gap-3 rounded-lg px-3 py-2.5 text-left text-xs font-extrabold transition-all relative ${
                      isActive
                        ? 'bg-blue-50/80 text-[#0B2E6B] before:absolute before:left-0 before:top-2 before:bottom-2 before:w-[3.5px] before:rounded-r-full before:bg-[#0B2E6B]'
                        : 'text-slate-700 hover:bg-slate-50'
                    }`}
                  >
                    <Icon
                      className={`h-4 w-4 shrink-0 transition-colors ${
                        isActive ? 'text-[#0B2E6B]' : 'text-slate-400 group-hover:text-slate-700'
                      }`}
                    />
                    <span className="flex-1 truncate">{cat.name}</span>
                    <span
                      className={`ml-auto inline-flex items-center justify-center rounded-full px-2 py-0.5 text-[10px] font-extrabold ${
                        isActive ? 'bg-white text-[#0B2E6B] shadow-xs' : 'bg-slate-100 text-slate-500'
                      }`}
                    >
                      {catCount}
                    </span>
          ...
          <div className="rounded-xl border border-blue-100 bg-blue-50/50 p-5 shadow-xs lg:w-[260px] xl:w-[280px]">
            <div className="mb-3 flex h-10 w-10 items-center justify-center rounded-full bg-white shadow-xs text-[#0B2E6B]">
              <BookOpen className="h-5 w-5" />
            </div>
            <h3 className="mb-1 text-sm font-extrabold text-[#0D1F3D]">Need Help?</h3>
            ...
            <Button
              ...
              className="w-full bg-white text-xs font-bold text-[#0D1F3D] border-slate-200 shadow-xs hover:bg-slate-50"
            >
              Learn More
            </Button>
          </div>
        ...
        <div className="flex min-w-0 flex-1 flex-col gap-4">
          ...
          <div className="flex-1 rounded-xl border border-slate-200 bg-white shadow-xs">
            ...
            <div className="border-b border-slate-100 p-5">
              <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
                <div>
                  <h2 className="text-lg font-extrabold text-[#0D1F3D]">{activeCategoryConfig.name}</h2>
                  <p className="mt-1 text-xs font-medium text-slate-500">{activeCategoryConfig.description}</p>
                </div>
                  ...
                  <div className="flex flex-col gap-3 sm:flex-row">
                    <div className="relative min-w-[240px]">
                      ...
                      <input
                        ...
                        className="h-10 w-full rounded-lg border border-slate-200 bg-white pl-9 pr-4 text-xs font-semibold text-[#0D1F3D] placeholder-slate-400 focus:border-[#0D1F3D] focus:outline-none"
                      />
                    </div>
                    ...
                    <div className="flex items-center gap-2">
                      <Button
                        ...
                        className={`h-10 gap-2 rounded-lg border-slate-200 px-4 text-xs font-bold ${
              ...
              {showFilters && (
                <div className="mt-4 grid grid-cols-1 gap-3 sm:grid-cols-2 pt-3 border-t border-slate-100">
                  <div>
                    <label className="text-[11px] font-bold text-slate-500 block mb-1">Filter by Activity</label>
                    <select
                      value={activityFilter}
                      onChange={(e) => setActivityFilter(e.target.value as any)}
                      className="h-9 w-full rounded-lg border border-slate-200 bg-white px-3 text-xs font-semibold text-[#0D1F3D] focus:border-[#0D1F3D] focus:outline-none"
                    >
                  ...
                  <div>
                    <label className="text-[11px] font-bold text-slate-500 block mb-1">Filter by Status Type</label>
                    <select
            ...
            <div className="p-5">
              <div className="mb-6 grid gap-3 sm:gap-4 grid-cols-2 lg:grid-cols-4">
                <div className="flex min-w-0 items-start gap-3 overflow-hidden rounded-xl border border-slate-200 bg-white p-3.5 shadow-xs">
                  <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-blue-50 text-[#0B2E6B]">
                    <Flag className="h-5 w-5" />
                  </span>
                  <div className="min-w-0 flex-1">
                    <p className="truncate text-xs font-bold text-slate-500">{activeCategoryConfig.totalLabel}</p>
                    <p className="truncate text-xl font-extrabold text-[#0D1F3D] mt-0.5">{stats.total}</p>
                  </div>
                </div>
                ...
                <div className="flex min-w-0 items-start gap-3 overflow-hidden rounded-xl border border-slate-200 bg-white p-3.5 shadow-xs">
                  ...
                  <div className="min-w-0 flex-1">
                    <p className="truncate text-xs font-bold text-slate-500">Active</p>
                    <p className="truncate text-xl font-extrabold text-[#0D1F3D] mt-0.5">{stats.active}</p>
                  </div>
                </div>
                ...
                <div className="flex min-w-0 items-start gap-3 overflow-hidden rounded-xl border border-slate-200 bg-white p-3.5 shadow-xs">
                  ...
                  <div className="min-w-0 flex-1">
                    <p className="truncate text-xs font-bold text-slate-500">Inactive</p>
                    <p className="truncate text-xl font-extrabold text-[#0D1F3D] mt-0.5">{stats.inactive}</p>
                  </div>
                </div>
                ...
                <div className="flex min-w-0 items-start gap-3 overflow-hidden rounded-xl border border-slate-200 bg-white p-3.5 shadow-xs">
                  ...
                  <div className="min-w-0 flex-1">
                    <p className="truncate text-xs font-bold text-slate-500">System Default</p>
                    <p className="truncate text-xl font-extrabold text-[#0D1F3D] mt-0.5">{stats.systemDefault}</p>
                  </div>
                </div>
              </div>
                      ...
                      {filteredRows.length > 0 ? (
                          ...
                          <tr key={row.id} className="hover:bg-slate-50/70 transition-colors">
                            ...
                            <td className="px-4 py-3">
                              <div className="flex items-center gap-2.5">
                                ...
                                <div>
                                  <p className="font-extrabold text-[#0D1F3D]">{row.name}</p>
                                  <p className="text-[10px] font-mono text-slate-400 mt-0.5">{row.code}</p>
                                </div>
                              </div>
                            </td>

                            {/* Description */}
                            <td className="px-4 py-3 text-slate-600 font-medium max-w-[280px]">
                              <p className="truncate">{row.description || '—'}</p>
                            </td>

                            {/* Sort Order */}
                            <td className="px-4 py-3 text-center">
                              <span className="rounded-md bg-slate-100 px-2 py-0.5 font-extrabold text-slate-700">
                                #{row.sortOrder}
                              </span>
                            </td>
                            ...
                            <td className="px-4 py-3">
                              {row.isSystemDefault ? (
                                <span className="inline-flex items-center gap-1 rounded-md bg-blue-50 px-2 py-0.5 text-[10px] font-extrabold text-[#0B2E6B] border border-blue-200">
                                  <Tag className="h-3 w-3 text-[#0B2E6B]" /> System Default
                                </span>
                              ) : (
                                <span className="inline-flex items-center gap-1 rounded-md bg-slate-100 px-2 py-0.5 text-[10px] font-extrabold text-slate-600 border border-slate-200">
                                  Custom
                                </span>
                              )}
                            </td>
                            ...
                            <td className="px-4 py-3">
                              <button
                                ...
                                className={`inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 text-[11px] font-extrabold transition-all cursor-pointer border ${
                                  row.isActive
                                    ? 'bg-emerald-50 text-emerald-700 border-emerald-200 hover:bg-emerald-100'
                                    : 'bg-slate-100 text-slate-500 border-slate-200 hover:bg-slate-200'
                                }`}
                              >
                                {row.isActive ? <CheckCircle2 className="h-3.5 w-3.5" /> : <PauseCircle className="h-3.5 w-3.5" />}
                            ...
                            <td className="px-4 py-3 text-right">
                              <div className="flex items-center justify-end gap-1.5">
                                <button
                                  ...
                                  className="rounded-lg border border-slate-200 bg-white p-1.5 text-slate-600 hover:border-[#0D1F3D] hover:text-[#0D1F3D] transition-colors"
                        ...
                        <tr>
                          <td colSpan={6} className="px-4 py-12 text-center text-slate-400">
                            ...
                            <p className="font-extrabold text-slate-700">No master records found</p>
          ...
          <div className="mt-2 flex items-center gap-3 rounded-xl bg-amber-50/80 border border-amber-200 px-4 py-3 text-xs text-amber-900">
      ...
      <Modal isOpen={modalOpen} onClose={() => setModalOpen(false)} maxWidth="max-w-md">
        <div className="space-y-4 font-sans text-xs">
          ...
          <div className="space-y-3.5">
            <div>
              <label className="font-bold text-slate-700 block mb-1">Master Record Name *</label>
              <input
                type="text"
                value={formName}
                onChange={(e) => setFormName(e.target.value)}
                placeholder="e.g. Senior Field Executive"
                className="w-full rounded-xl border border-slate-200 bg-white p-2.5 text-xs font-bold text-[#0D1F3D] focus:border-[#0D1F3D] focus:outline-none"
              />
            </div>

            <div>
              <label className="font-bold text-slate-700 block mb-1">System Code Key *</label>
              <input
                type="text"
                value={formCode}
                onChange={(e) => setFormCode(e.target.value)}
                placeholder="e.g. SR_FIELD_EXEC"
                className="w-full rounded-xl border border-slate-200 bg-white p-2.5 text-xs font-mono font-bold text-[#0D1F3D] focus:border-[#0D1F3D] focus:outline-none uppercase"
              />
            </div>
            ...
            <div>
              ...
              <textarea
                ...
                className="w-full rounded-xl border border-slate-200 bg-white p-2.5 text-xs font-medium text-[#0D1F3D] focus:border-[#0D1F3D] focus:outline-none"
              />
            </div>

            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="font-bold text-slate-700 block mb-1">Display Color Tag</label>
                <div className="flex items-center gap-2">
                  <input
                    type="color"
                    value={formColor}
                  ...
                  <input
                    ...
                    value={formColor}
                    onChange={(e) => setFormColor(e.target.value)}
                    className="w-full rounded-xl border border-slate-200 bg-white p-2 text-xs font-mono font-bold text-[#0D1F3D]"
                  />
                </div>
              </div>
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full rounded-xl border border-slate-200 bg-white p-2.5 text-xs font-extrabold text-[#0D1F3D] focus:border-[#0D1F3D] focus:outline-none"
                />
              </div>
            </div>
            ...
            <label className="flex items-center gap-2.5 rounded-xl border border-slate-200 bg-slate-50/70 p-3 cursor-pointer">
              ...
              />
              <div>
                <p className="font-extrabold text-[#0D1F3D]">Active Status</p>
                <p className="text-[10px] text-slate-400 font-medium">Active masters appear in application dropdowns and forms.</p>
              </div>
            </label>
          </div>
      ...
      <Modal isOpen={Boolean(deleteTarget)} onClose={() => setDeleteTarget(null)} maxWidth="max-w-sm">
        <div className="space-y-4 font-sans text-xs">
          <div className="flex items-start gap-3">
            ...
            <div>
              <h3 className="text-base font-extrabold text-[#0D1F3D]">Delete Master Record?</h3>
              <p className="text-xs text-slate-500 font-medium mt-1">
                Are you sure you want to delete <span className="font-extrabold text-rose-700">{deleteTarget?.name}</span>? This action cannot be undone.
              </p>
            </div>
          </div>
