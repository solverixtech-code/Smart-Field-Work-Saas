...
const permissionCode = "crm.visits.schedule";

export async function syncVisitSchedulingPermission(prisma: PrismaClient) {
  return prisma.$transaction(async (tx) => {
    const definition = PERMISSION_REGISTRY.find(
      (entry) => entry.code === permissionCode,
    );
    ...
    const roleCodes = Object.entries(DEFAULT_TENANT_ROLE_GRANTS)
      .filter(([, grants]) => grants.includes(permissionCode))
...
if (require.main === module) {
  ...
  syncVisitSchedulingPermission(prisma)
