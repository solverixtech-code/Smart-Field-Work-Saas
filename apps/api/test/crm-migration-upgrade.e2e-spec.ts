...
describe.each([
  {
    count: 33,
    file: "phase-0-migration-hashes.json",
    addition: "20260915110000_phase_1_1_account_contact",
  },
  {
    count: 34,
    file: "phase-1-1-migration-hashes.json",
    addition: "20260915160000_phase_1_2_lead_manual_assignment",
  },
])(
  "CRM upgrade from $count historical migrations",
  ({ count, file, addition }) => {
    ...
    it("preserves every frozen SQL hash", () => {
      expect(Object.keys(frozenHashes.files)).toHaveLength(count);
    ...
    it("upgrades the populated baseline through exactly one new migration and preserves relationships", async () => {
      ...
      try {
        ...
        ).toEqual([{ name: count === 33 ? null : '"Account"' }]);
        ...
        const preservedAccount =
          count === 34
            ...
            : null;
        const preservedContact = preservedAccount
          ? await client.contact.create({
              data: {
                tenantId: tenant.id,
                accountId: preservedAccount.id,
                name: "Preserved contact",
                email: "preserved@test.invalid",
                createdByMembershipId: membership.id,
                updatedByMembershipId: membership.id,
              },
              select: { id: true, accountId: true, revision: true },
            })
          : null;
        ...
        const additions = readdirSync("prisma/migrations", {
          withFileTypes: true,
        }).filter((d) => d.isDirectory() && d.name === addition);
        ...
        if (preservedAccount && preservedContact) {
          expect(
            await client.account.findUnique({
              where: { id: preservedAccount.id },
              select: { id: true, name: true, revision: true },
            }),
          ).toEqual(preservedAccount);
          expect(
            await client.contact.findUnique({
              where: { id: preservedContact.id },
              select: { id: true, accountId: true, revision: true },
            }),
          ).toEqual(preservedContact);
          const lead = await client.lead.create({
            data: {
              tenantId: tenant.id,
              name: "Upgraded lead",
              accountId: preservedAccount.id,
              contactId: preservedContact.id,
              ownerMembershipId: membership.id,
              createdByMembershipId: membership.id,
              updatedByMembershipId: membership.id,
            },
