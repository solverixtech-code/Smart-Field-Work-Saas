...
Counts: 23 Generic Masters (A), 9 Domain/Commercial Entities (B/D), 10 Policies (C), 1 deprecated duplicate (`shift_type`), plus the remaining target/workflow rows accounted for above. The exact class label on each row is authoritative; counts should be recalculated automatically if classifications change.
